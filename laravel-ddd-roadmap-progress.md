...
完了条件:
- `StoreTaskTest` の4本が通る

#### 課題2: complete の仕様を固定する
対象:
- `CompleteTaskTest`
...
狙い:
- `'todo'`, `'done'` のマジックストリングを減らす
- `status` を意味ある型として扱う

#### 課題4: `complete()` を Task の振る舞いへ寄せる
対象:
- `Task::complete()`
...
#### 課題5: `Task::complete()` の Unit Test を追加する
対象:
...
次の改善候補:
- `Task::complete()` で `update()` を使わず、状態変更だけを行う
...
候補:
- `DueDate` を Value Object 化する
- `CreateTaskUseCase`
- `CompleteTaskUseCase`

この段階で考えること:
- `due_date` は単なる日付か
- `Task` の生成や完了はどこが責務を持つべきか
...
#### テスト
- `StoreTaskTest` 4本すべて通過
    - 今日の日付の期限ならタスクを作成できる
    - 期限日なしでもタスクを作成できる
    - 過去日付の期限ではタスクを作成できない
    - タイトルが空なら作成できない

- `CompleteTaskTest` 2本すべて通過
    - 未完了タスクは完了できる
    - 完了済みタスクは再度完了できない

#### DDD 入口
- `TaskStatus` enum 導入済み
- `Task` モデルで enum cast 利用中
- Blade でも enum を使う方向へ修正済み
- `TaskController@complete()` から `Task::complete()` を呼ぶ構成へ移行済み

#### Unit Test
- `tests/Unit/TaskTest.php` 作成済み
- 以下の2本を追加済み
    - 未完了タスクは完了できる
    - 完了済みタスクは再度完了できない
...
- 「文字列」から「enum」
- 「Controller の条件分岐」から「Task の振る舞い」
へ寄せ始めている
...
### 優先度高
1. `TaskTest` を通しつつ、`Task::complete()` の責務を整理する
2. `Task::complete()` の中で `update()` ではなく状態変更だけを行う形へ寄せる
3. Controller 側で `save()` する形へ寄せる
4. すべてのテストを再実行して通す

### その次
5. `DomainException` を専用例外に置き換えるか検討する
6. `due_date` を Value Object 候補として整理する
7. `CreateTaskUseCase` / `CompleteTaskUseCase` を導入するか検討する
